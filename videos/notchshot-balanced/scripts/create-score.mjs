...
const sr=44100, duration=58, n=Math.ceil(sr*duration), left=new Float32Array(n), right=new Float32Array(n);
const beat=60/116, midi=m=>440*2**((m-69)/12);
...
for(let bar=0;bar<29;bar++){
 const at=bar*beat*4;if(at>56.5)break; const c=chords[bar%4], level=bar>26?.7:1;
...
[57,60,64,69,76].forEach((m,i)=>pluck(54+i*.045,m+12,.075,(i-2)*.16));
...
let peak=0;for(let i=0;i<n;i++){const t=i/sr,fade=Math.min(1,t/.06,Math.max(0,(duration-t)/2));left[i]*=fade;right[i]*=fade;peak=Math.max(peak,Math.abs(left[i]),Math.abs(right[i]));}
